    ...
    if($validation->passed())
    {
      ...
      try
      {
        $user->create(array(
            ...
            'emailcode' => Hash::make(Input::get('name'),Hash::salt(32)),
            'active'    => 0
        ));
      }
      catch(Exception $e)
      ...
      ?>
        ...
        <form method="POST" name="form2" action="http://www.aasf.in/mailer/regmail.php">
          <input type="hidden" name="name" value="<?php echo $user->data()->name?>">
          <input type="hidden" name="email" value="<?php echo $user->data()->email?>">
          <input type="hidden" name="emailcode" value="<?php echo $user->data()->emailcode?>">
